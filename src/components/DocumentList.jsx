                    ...
                    <div key={doc.id} className="relative bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white rounded-2xl border border-gray-300 dark:border-gray-600 shadow-md hover:shadow-lg transition-shadow p-5 flex flex-col justify-between">
                        <div>
                            ...
                            <div className="flex gap-2 flex-wrap mt-2">
                                <button onClick={() => fetchVersions(doc.id)} className="text-sm text-blue-600 hover:underline">
                                    {t("showVersions")}
                                </button>
                                <button onClick={() => setModalOpenFor(doc.id)} className="text-sm text-green-600 hover:underline">
                                    {t("uploadNewVersion")}
                                </button>
                            </div>
                                ...
                                <ul className="mt-2 text-sm text-gray-600 dark:text-gray-300">
                                    {versionsByDocId[doc.id].map(v => (
                                        <li key={v.id} className="flex justify-between items-center">
                                            ...
                                            <button
                                                onClick={() => handleDownload(v.id, `${v.title || "version"}.pdf`)}
                        ...
                        <button onClick={() => handleDownload(doc.id, `${doc.title || "document"}.pdf`)} className="absolute top-3 right-3 text-blue-500 dark:text-blue-300 hover:underline text-sm">
                            {t("download")}
                        </button>
